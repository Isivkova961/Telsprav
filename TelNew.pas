  ...
  if fTelSprav.NewEdit=true then
    begin
      with dmTel.adoqTel do
        begin
          Insert;
          FieldByName('Фамилия').Value:=eFam.Text;
          FieldByName('Имя').Value:=eIm.Text;
          ...
          FieldByName('Примечание').Value:=eText.Text;
          Post;
        end;
    end
  else
...
begin
  if not (Key in ['а'..'я','А'..'Я',#8,#46]) then Key:=#0;
end;
...
begin
  ...
  with dmTel.adoqTel do
    begin
      ...
      eIm.Text:=FieldByName('Имя').AsString;
      eOt.Text:=FieldByName('Отчество').AsString;
      eDepart.Text:=FieldByName('Отделение').AsString;
      ePost.Text:=FieldByName('Должность').AsString;
      eText.Text:=FieldByName('Примечание').AsString;
      meTel.Text:=FieldByName('Телефон').AsString;
      ...
      eNameComp.Text:=FieldByName('Имя компьютера').AsString;
    end;
end;
